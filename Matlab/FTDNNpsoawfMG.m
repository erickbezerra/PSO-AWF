...
% Normalizacao
gasx = mgdata(1:450,1)';
gasy = mgdata(1:451,1)'; % Defino variavel y
maxgasx = max(gasx);
mingasx = min(gasx);
...
Comeca = 19; % Como sao 7 delays em um intervalo de 12 pontos, comeca a previsao no 13
...
% Definicao da Pbest incial e Gbest inicial
    for ii = 1:pop
        for i=1:(Neto-Comeca) % Pq como sempre preve o prox, no ultimo não teria o desejado para calcular o erro
            ...
            Inpz = [ 1 dgasx(i) dgasx(i+5) dgasx(i+11) dgasx(i+17) ];
            uz = Inpz*w{ii}';
...
% Normalizacao
gasxV = mgdata(451:951,1)';
gasyV = mgdata(451:952,1)'; % Defino variavel ymaxgasxV = max(gasxV);
maxgasxV = max(gasxV);
mingasxV = min(gasxV);
...
for i=1:(Neto2-Comeca) %Pq como sempre preve o prox, no ultimo não teria o desejado para calcular o erro
    % Calcular entradas para funcao de ativacao
    Inpz = [ 1 dgasxV(i) dgasxV(i+5) dgasxV(i+11) dgasxV(i+17) ];% 1 Devido ao Bias e outro a faixa de previsao de 1h
        uz = Inpz*w';
        % Calculada saida da funcao de ativacao Sigmoide Logistica
        for uzd = 1:NNCI
        z(uzd) = 1/(1+exp(-uz(1,uzd)));
...
outfile = ['psoawf1N1000IartigoMG',int2str(sn-1)];
save(outfile);
sn = sn+1;
end
