...
% Normalizacao
gasx = gas_furnaceT(:,1)';
gasy = gas_furnaceT(:,2)'; % Defino variavel y
maxgasx = max(gasx);
mingasx = min(gasx);
...
Ninb = 3; % Numero de entradas da rede neural
Comeca = 5; % Como sao 7 delays em um intervalo de 12 pontos, comeca a previsao no 13
...
WF = 5; % Valor do WF
ParticBorn = 480; % Numero de particulas geradas a cada geracao
...
% Normalizacao
gasxV = gas_furnaceV(:,1)';
gasyV = gas_furnaceV(:,2)'; % Defino variavel y
maxgasxV = max(gasxV);
mingasxV = min(gasxV);
...
for i=1:(Neto2-Comeca) %Pq como sempre preve o prox, no ultimo não teria o desejado para calcular o erro
    % Calcular entradas para funcao de ativacao
    Inpz = [ 1 dgasxV(i) dgasxV(i+3) ];% 1 Devido ao Bias e outro a faixa de previsao de 1h
        uz = Inpz*w';
        % Calculada saida da funcao de ativacao Sigmoide Logistica
        for uzd = 1:NNCI
        z(uzd) = 1/(1+exp(-uz(1,uzd)));
...
outfile = ['psoawf1N1000IartigoBJ',int2str(sn-1)];
save(outfile);
sn = sn+1;
end
